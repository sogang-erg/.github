<p align="center">
  <img src="https://raw.githubusercontent.com/sogang-erg/.github/main/profile/assets/kermit.gif" height="220">
  <img src="https://raw.githubusercontent.com/sogang-erg/.github/main/profile/assets/kermit-2.gif" height="220">
</p>

<h1 align="center">Experiential Reality Group</h1>

<h3 align="center">
  Dept. of Art & Technology, Sogang University
</h3>

<p align="center">
  Extended Reality (XR) / Human-Centered AI / Game Technology
</p>

<p align="center">
  <a href="https://qoocrab-studio.vercel.app/">ERG Website</a> |
  <a href="https://qoocrab-studio.vercel.app/#publications">Publications</a> |
  <a href="https://creative.sogang.ac.kr/">Art & Technology</a> |
  <a href="mailto:qoocrab@sogang.ac.kr">Contact</a>
</p>

---

## About

ERG is a research group in the Dept. of Art & Technology at Sogang University, Korea's first department. We study how immersive experiences work, and how to make them work better. From empirical studies of XR presence and perception to real-time 3D and neural rendering, we use games as our research platform to build experiences that feel vivid, comfortable, and shared.

## Research

**Extended Reality (XR)**
When does a virtual moment feel real? Studying how presence is formed, broken, and recovered.

**Human-Centered AI**
Intelligent systems that read and align with human experience in real time.

**Game Technology**
Games as laboratory and muse, powered by real-time 3D and neural rendering.

## Themes

| | |
|---|---|
| **XR Presence & AI Interpretation** | We measure gaze, motion, and behavior, and train AI models to read how presence is formed, broken, and recovered. |
| **Adaptive XR with AI** | Virtual worlds that sense the user and let AI adjust them in real time, keeping immersion high and discomfort low. |
| **Social & Multi-User XR** | How people work, play, and recover together in shared virtual spaces, and how AI can mediate what each person sees and when to step in. |
| **Game Technology & AI Rendering** | Real-time 3D worlds built and rendered with AI, with games as both our laboratory and our inspiration. |

## Join Us

We are recruiting **Ph.D. students**, **Master's students**, and **undergraduate interns** from Art & Technology and beyond.

If you are interested, email [qoocrab@sogang.ac.kr](mailto:qoocrab@sogang.ac.kr) with your CV and the research topics you care about.

<p align="center">
  <sub>Led by Prof. Hayeon Kim, Dept. of Art & Technology, Sogang University</sub>
</p>
